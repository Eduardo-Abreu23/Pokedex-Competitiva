...
// effectiveness[attacker][defender] = multiplier (only non-neutral values stored)
const CHART: Partial<Record<PokemonTypeName, Partial<Record<PokemonTypeName, number>>>> = {
...
export function computeDefensiveProfile(types: PokemonTypeName[]): TypeEffectiveness[] {
  return ALL_TYPES.map((attacker) => {
    const mult = types.reduce((acc, defender) => {
      return acc * (CHART[attacker]?.[defender] ?? 1);
    }, 1);
